...
CREATE OR REPLACE FUNCTION private.is_congresso_member(_congresso_id uuid, _user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION private.can_add_chat_room_member(_room_id uuid, _member_user_id uuid, _actor_user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT EXISTS (
    ...
    WHERE cr.id = _room_id
      ...
      AND private.is_congresso_member(cr.congresso_id, _member_user_id)
      AND private.is_congresso_member(cr.congresso_id, _actor_user_id)
  );
$$;
...
USING (
  auth.uid() = created_by
  OR private.is_chat_room_member(id, auth.uid())
);
...
WITH CHECK (
  auth.uid() = created_by
  AND private.is_congresso_member(congresso_id, auth.uid())
);
...
TO authenticated
USING (
  private.is_chat_room_member(room_id, auth.uid())
  OR EXISTS (
    SELECT 1
    FROM public.chat_rooms cr
    WHERE cr.id = room_id
...
TO authenticated
WITH CHECK (
  private.can_add_chat_room_member(room_id, user_id, auth.uid())
);
